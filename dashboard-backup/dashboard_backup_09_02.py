...
df = pd.read_csv("out.csv.gz",  
           compression="gzip")

df_indices = pd.read_csv("daily_indices_82_to_19.csv",usecols=["timestamp","nao", "ao","mjo80e","mjo40w","mjo20e","mjo160e","mjo10w","nino34"])
...
for country in country_name_to_iso2.values():
    ...
    #     (daily_cp_dict_entire_period[country]['capacity_factor_w'] > i).mean() for i in np.linspace(0, 1, 100)]
...
def compute_combined_cp(value=100) :
    wind_share = value/100
    assert (wind_share <= 1 and wind_share >=0)
    
    global daily_cp_df
    global daily_LP_df
    global df_prices_dict


    df_hourly_all_years.loc[:,"capacity_factor"] = wind_share * df_hourly_all_years["capacity_factor_w"] + (1-wind_share) * df_hourly_all_years["capacity_factor_s"]
    df_month_all_years.loc[:,"capacity_factor"] = wind_share * df_month_all_years["capacity_factor_w"] + (1-wind_share) * df_month_all_years["capacity_factor_s"]
    df_dict["hour"].loc[:,"capacity_factor"] = wind_share * df_dict["hour"]["capacity_factor_w"] + (1-wind_share) * df_dict["hour"]["capacity_factor_s"]
    df_dict["month"].loc[:,"capacity_factor"] =  wind_share * df_dict["month"]["capacity_factor_w"] + (1-wind_share) * df_dict["month"]["capacity_factor_s"]
    df_dict["year"].loc[:,"capacity_factor"] =  wind_share * df_dict["year"]["capacity_factor_w"] + (1-wind_share) * df_dict["year"]["capacity_factor_s"]
    df_daily_cp.loc[:,"capacity_factor"] =  wind_share * df_daily_cp["capacity_factor_w"] + (1-wind_share) * df_daily_cp["capacity_factor_s"]
    daily_cp_eu_entire_period.loc[:,"capacity_factor"] =  wind_share * daily_cp_eu_entire_period["capacity_factor_w"] + (1-wind_share) * daily_cp_eu_entire_period["capacity_factor_s"]
    monthly_cp_eu.loc[:,"capacity_factor"] =  wind_share * monthly_cp_eu["capacity_factor_w"] + (1-wind_share) * monthly_cp_eu["capacity_factor_s"]

    for country in country_name_to_iso2.values():
        daily_cp_dict_entire_period[country].loc[:,"capacity_factor"] = wind_share * daily_cp_dict_entire_period[country]["capacity_factor_w"] + (1-wind_share) * daily_cp_dict_entire_period[country]["capacity_factor_s"]
    ...
    daily_LP_df = pd.DataFrame(np.array(
    ...
    for country_iso in country_name_to_iso2.values() :
        df_prices_dict[country_iso] = pd.merge(left = daily_cp_dict_entire_period[country_iso].loc[:,["day","capacity_factor_w"]], right = df_prices[df_prices["Country"]==country_iso], left_on = "day", right_on="timestamp")

    return 'The displayed data is for a mix of {}% wind and {}% solar'.format(value,100-value)
...
app.layout = html.Div([
    ...
    html.Div([dcc.Slider(0, 100, 10,value=100, marks={
                        value: {"label": "{}/{}".format(value,100-value)} for value in range(0,110,10) }, id='my-slider')], style={"width": "40vw"}),
    
    ], style={"height": "10vh", "display": "flex", "justifyContent": "center","flexDirection":"row","alignItems":"center"}),
    html.Div([
    dbc.Col(
        dbc.Card(
            dbc.CardBody([
                ...
                dbc.Row([
                    ...
                    dbc.Col([
                        dcc.Dropdown(options=[{'label': x, 'value': x} for x in range(
                            1979, 2020)], value=2019, clearable=False, searchable=False, id="year_picker")], id="year_picker_div", style={"display": "none"}),
...
], style={"height": "100vh", "display": "flex", "justifyContent": "center", "flexDirection": "row"}), #align items center
...
dbc.Modal(
            [
                ...
                dbc.ModalBody([
                    ...
                    html.Br(),
                    html.P("LP events: Short for Low Power events. Consecutive days during which the daily capacity factor is constantly below a threshold of 10 per cent.",style={"textAlign": "left"})]),
            ],
            id="modal-lg",
            size="lg",
            is_open=False,
...
def update_output(value):
    children = compute_combined_cp(value)
    return children
...
def update_map(period_radio, avg_std, range,_):
...
def update_plot(period_radio, range, click,_):
    if range[0] == range[1]:
        return
...
def update_side_graph(click, fig_choice, year,_):
    ...
    if fig_choice == "Inter-annual variability of the number of LP days" :
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=[], y=[]))
        lp = daily_cp_eu_entire_period[["year","capacity_factor"]]
        lp['count'] = (lp.capacity_factor<0.1)
        lp = lp[['count','year']].groupby("year").sum().reset_index()
        fig.add_trace(go.Scatter(x=lp["year"], y=lp["count"], name="28C",line_shape='linear', 
        showlegend=True,
        ...
        if click :
            ...
            lp_count_per_year_dict = dict()
            for selected_country in selected_countries:
                lp = df_daily_cp[df_daily_cp.country==selected_country][["year","capacity_factor"]]
                lp['count'] = (lp.capacity_factor<0.1)
                lp = lp[['count','year']].groupby("year").sum().reset_index()
                lp_count_per_year_dict[selected_country] = lp
            
            for selected_country in selected_countries:
                fig.add_trace(go.Scatter(x=lp_count_per_year_dict[selected_country]["year"], y=lp_count_per_year_dict[selected_country]["count"], name = selected_country, showlegend=True))
    ...
    if fig_choice == "Day-ahead prices (Daily average)" :
        ...
        if click is None :
            ...
            fig.add_trace(go.Scatter(x=df_prices_europe.timestamp, y=df_prices_europe.price,name = "28C", line={'color': "rgb(187,51,59)","width":1}, showlegend=True, visible=True))
            ...
            for country_iso in selected_countries :
                ...
                if len(df_prices_dict[country_iso]) == 0 :             
                    return [html.Div("No data for {}".format(country_iso), style={"marginBottom": "10vh", "marginTop": "10vh", "textAlign": "center"})], {"display": "none"}, {"display": "none"}

            df_concat = pd.concat([df_prices_dict[country_iso] for country_iso in selected_countries])
            by_row_index = df_concat.groupby([df_concat.timestamp])
            df_means = by_row_index.mean().reset_index()
            name = (click["points"][0]["location"] if len(click["points"]) == 1 else "selected region ")
            color = ('rgb(6, 168, 67)' if len(click["points"]) == 1 else 'grey')
            corr = df_means.price.corr(df_means.capacity_factor_w)
            fig.add_trace(go.Scatter(x=df_means.timestamp, y=df_means.price,name = name , visible=True,line_shape='linear', showlegend=True, line={'color': color,"width":0.5}))
            fig.add_trace(go.Scatter(x=df_means.timestamp, y=df_means.price.where(df_means.capacity_factor_w<0.1), name = "Prices during low <b>wind</b> power days.<br>Correlation between <b>wind</b> capacity factor and electricity price ({}-{}) is {:.2f}".format(df_means.timestamp.iloc[0][:4],df_means.timestamp.iloc[-1][:4], corr), mode='markers', marker=dict(color='red',size=3), showlegend=True,visible="legendonly"))

        
        #     fig.add_annotation(text="Correlation between <b>wind</b> capacity factor and electricity price ({}-{}) is {:.2f}".format(df_means.timestamp.iloc[0][:4],df_means.timestamp.iloc[-1][:4], corr),
        # xref="paper", yref="paper",
        # x=0, y=0, showarrow=False,font=dict(size=10,color="red"))
    ...
    if fig_choice == 'Spatial correlation between daily capacity factor':
        ...
        if not click or len(click["points"]) != 1:
            return html.Div("Select only one country on the left map to display the correlation between the LP day distribution of this country and the LP day distribution of the other European countries", style={"marginTop": "10vh", "textAlign": "center"}), {"display": "none"}, {"display": "none"}
    ...
    if fig_choice == 'Spatial correlation between LP day distribution':
        ...
        if not click or len(click["points"]) != 1:
            return html.Div("Select one country on the left map to display the correlation between the LP day distribution of this country and the LP day distribution of the other European countries", style={"marginTop": "10vh", "textAlign": "center"}), {"display": "none"}, {"display": "none"}
        selected_countries = click["points"][0]['location']
        rho = abs(daily_LP_df.corr()[selected_countries])
        pval = (daily_LP_df.corr(method=lambda x, y: pearsonr(x, y)[1]) - np.eye(*rho.shape))[selected_countries] 
        rho[pval>0.05]=np.NaN
        fig = px.choropleth_mapbox(rho,
                                   geojson="https://raw.githubusercontent.com/leakyMirror/map-of-europe/master/GeoJSON/europe.geojson",
                                   featureidkey='properties.ISO2',
    ...
    if fig_choice == "Climate indices and LP events" :
        ...
        if not click or len(click["points"]) < 1:
            return [html.Div("Select a region on the left map to display the number of low power (LP) events during the selected year", style={"marginBottom": "10vh", "marginTop": "10vh", "textAlign": "center"}),
                    html.Br(),
                    gif.GifPlayer(
                gif='assets/rec.gif',
                still='assets/rec.png',
        ...
        if year < 1982 :
            return [html.Div("No climate data for this year. Please choose a year between 1982 and 2019", style={"marginBottom": "10vh", "marginTop": "10vh", "textAlign": "center"}),
                    ], {"display": "none"}, {}
        ...
        merge = pd.merge(left=df_means,right=df_indices,right_on="timestamp", left_on="day")


        fig = go.Figure()

        for index in indices :
            fig.add_trace(go.Scatter(x=merge.timestamp, y=merge[index], line={'color': 'grey',"width":1}, name = index,legendgroup=index, showlegend=False, visible=(False if index!="nao" else True)))
            fig.add_trace(go.Scatter(x=merge.timestamp, y=merge[index].where(merge.capacity_factor<0.1), mode='markers',name = index+" during LP days",marker=dict(color='red',size=5), showlegend=False,legendgroup=index,visible=(False if index!="nao" else True)))
                    ...
                    "buttons": [
                        {
                            ...
                            "method": "update",
                            "args": [
                                {"visible": [c == c2 for c2 in indices for _ in range(2) ]}
                            ],
                        }
                        for c in indices
                    ]
        ...
        fig.add_annotation(text="climate index values during lp days are indicated in red",
            xref="paper", yref="paper",
            x=0, y=0, showarrow=False,font=dict(size=10,color="red"))
